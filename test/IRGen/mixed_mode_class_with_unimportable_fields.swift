...
// RUN: %target-swift-frontend -emit-ir -I %t -I %S/Inputs/mixed_mode -module-name main -swift-version 4 %s | %FileCheck %s --check-prefix=CHECK --check-prefix=CHECK-%target-ptrsize --check-prefix=CHECK-V4 -DWORD=i%target-ptrsize --check-prefix=CHECK-V4-DEP-TARGET-GTE-13_0-%target-mandates-deployment-target-gte-13.0
// RUN: %target-swift-frontend -emit-ir -I %t -I %S/Inputs/mixed_mode -module-name main -swift-version 5 %s | %FileCheck %s --check-prefix=CHECK --check-prefix=CHECK-%target-ptrsize --check-prefix=CHECK-V5 -DWORD=i%target-ptrsize --check-prefix=CHECK-V5-DEP-TARGET-GTE-13_0-%target-mandates-deployment-target-gte-13.0
...
// CHECK-V4-DEP-TARGET-GTE-13.0-TRUE:    call swiftcc %swift.metadata_response @swift_updateClassMetadata2(
// CHECK-V4-DEP-TARGET-GTE-13.0-FALSE:   call swiftcc %swift.metadata_response @swift_initClassMetadata2(
